<script lang="ts">
    ...
    onMount(async () => {
        ...
        .then( data => {moviePosters = data.posters, movieLogos = data.logos} )
        .catch(error => {
            console.log(error);
            return [];
        });
...
<div class="movie-slide flex-1 bg-cover bg-no-repeat bg-center flex justify-center items-end overflow-hidden transition-all duration-1000 relative" style="background-image: url(https://image.tmdb.org/t/p/original{movie.backdrop_path});">
    ...
    {/each}
    <div class="movie-content text-white absolute w-full h-full opacity-0 bg-cyan-950 bg-opacity-60 grid grid-cols-2 gap-56 items-center transition-all duration-1000 ">
        <div class="movie-exerpt p-14">
